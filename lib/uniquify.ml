...
(* Exercise 2.1 *)
let rec uniquify_expr env = function
  | Var x -> Var (StrMap.find x env)
  | Prim1 {op; e} -> Prim1 {op; e = uniquify_expr env e}
  | Prim2 {op; l; r} -> Prim2 {op; l = uniquify_expr env l; r = uniquify_expr env r}
  | Let {v; e; body} ->
    incr i;
    let name = v ^ "_" ^ string_of_int !i in
    let env = StrMap.add v name env in
    Let {
      v = name ;
...
let uniquify {body} =
  i := 0;
  {body = uniquify_expr StrMap.empty body}
